...
import {
  CATEGORY_VALIDATION,
  DESCRIBTION_VALIDATION,
  PRICE_VALIDATION,
  RECIPE_NAME_VALIDATION,
  TAG_VALIDATION,
      ...
      if (isEdit) {
        await axiosInstance.put(`${RECIPE_API}/${id}`, recipeData, {
          headers: { "Content-Type": "multipart/form-data" },
              ...
              console.log("Success toast should show now"); // Debug
  ...
  const getAllCategories = async () => {
    try {
      const response = await axiosInstance.get(
        `${CATEGORY_API}?pageSize=50&pageNumber=1`
      );
  ...
  return (
    <div>
      ...
      >
        ...
        {/* Recipe Name */}
        <input
          {...register("name", RECIPE_NAME_VALIDATION)}
          type="text"
        ...
        <div className="my-2">
          <select
            {...register("categoryId", CATEGORY_VALIDATION)}
            className="form-control bg-light"
          >
        ...
        {/* Description */}
        <textarea
          {...register("description", DESCRIBTION_VALIDATION)}
          className="form-control my-2 bg-light"
